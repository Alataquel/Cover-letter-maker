...
export const CoverLetterCard: React.FC<CoverLetterCardProps> = ({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'generated': return 'bg-primary text-primary-foreground';
      case 'edited': return 'bg-accent text-accent-foreground';
      default: return 'bg-muted text-muted-foreground';
    }
  };

  return (
    <Card className="bg-gradient-card shadow-card hover:shadow-card-hover transition-smooth transform hover:-translate-y-1 cursor-pointer group p-6">
