...
export const Dashboard: React.FC = () => {
  ...
  const filteredCoverLetters = coverLetters.filter(letter => letter.title.toLowerCase().includes(searchQuery.toLowerCase()) || letter.companyName.toLowerCase().includes(searchQuery.toLowerCase()) || letter.position.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <header className="border-b border-border bg-gradient-card shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => setShowForm(true)} variant="professional" size="lg" className="shadow-button hover:shadow-card-hover transform hover:-translate-y-0.5">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="relative max-w-md">
            ...
            <Input placeholder="Search cover letters..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
          </div>
        </div>

        {/* Cover Letters Grid */}
        {filteredCoverLetters.length === 0 ? <Card className="bg-gradient-card shadow-card p-12 text-center">
            <div className="max-w-md mx-auto">
              ...
              <p className="text-muted-foreground mb-6">
                {searchQuery ? 'Try adjusting your search terms' : 'Create your first AI-powered cover letter to get started'}
              </p>
              {!searchQuery && <Button onClick={() => setShowForm(true)} variant="professional" className="shadow-button">
                  <Plus className="h-4 w-4" />
                  Create Your First Cover Letter
                </Button>}
            </div>
          </Card> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCoverLetters.map(letter => <CoverLetterCard key={letter.id} {...letter} onEdit={handleEditCoverLetter} onDelete={handleDeleteCoverLetter} onView={handleViewCoverLetter} />)}
          </div>}
      </main>

      {/* Cover Letter Form Modal */}
      {showForm && <CoverLetterForm onClose={() => setShowForm(false)} onSave={handleSaveCoverLetter} />}
    </div>;
};